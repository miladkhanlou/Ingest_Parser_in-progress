...
def xml2workbench(DataFrame, mods):   
    ...
    pathName = []
    field_with_text = {}  #Final dictionary
    for mod in mods:
        ...
        # path_list = [] #list of paths
        # result_dict = {} #the paths with the text in them with the name of paths
        # pathName = []
        # field_with_text = {}  #Final dictionary

        #Get the PID field:
        field_with_text.setdefault("PID", [])
        ...
        for a,elem in root:
            if a == 'start':
                ...
                if path in path_list and elem.text is not None:
            ...
            for Xpaths in list(DataFrame["XMLPath"]):
                if paths in Xpaths:
                    ...
                    if values is not None:
                        ##Dictionary##
                        field_with_text.setdefault(fieldName, [])
                        for value in values: 
        ...
        for key in field_with_text:
            joined_string = '| '.join(str(item) for item in field_with_text[key])
            field_with_text[key] = joined_string
            
        print(" <----------------------test Dictionary--------------------------> ")
        print(field_with_text)
        #evening number of rows to be same length
    #     max_count = max(len(dict_value)for dict_value in field_with_text.values())
    #     for dict_key in field_with_text:
    #         dictValue = field_with_text[dict_key]
    #         if len(dictValue) < max_count:
    #             field_with_text[dict_key] = dictValue + [''] * (max_count - len(dictValue))
        
    #     #check the lengths:
    #     for a in field_with_text:
    #         b = field_with_text[a]
    #         # print(len(b))
    # print(field_with_text)
    # xml2wb_df = pd.DataFrame(field_with_text)

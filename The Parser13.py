...
def xml2workbench(root,data_frame):   
    ...
    for event,elem in root:
        if event == 'start':
            ...
            if tag_count[tag_name] == 2:
                second_tag_name = tag_name
            ...
            if path in path_list and elem.text is not None and elem.text.strip() != "":
                ...
                continue
        elif event == 'end' and elem.tag == second_tag_name:
            result_dict[path] = "|".join(text)
            print(result_dict[path])
            text = []
            pathName.pop()
    for i, j in result_dict.items():
        print("{} --------->>> {}".format(i,j))    ##compare xml paths we created from the directory of mods to the master csv we parsed before, and write the texts from value of result_dict dictionary to the field defined in master csv        
    Field_from_csv = []

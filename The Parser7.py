            ...
            if a == 'start':
                ...
                if len(attributes) > 0:
                    ...
                    ### A1) check for any miss-speling in tags and attributes
                            ...
                            if elem.tag.split("}")[1] not in csv_input["tags"]:
...
def xml2wb(fromCSV, paths_in_xml):
    ...
    xml2wb_df = xml2wb_df[xml2wb_df['needed'] != 'No']
...
def main():
    ...
    elif args.input_csv and args.output_directory:
    # Run the function to for xml2wirkbencg process (-i,-o,-c)
        ...
        sourceMODs = MODs(args)
        imput_csv = inpute_csv(args)
        parseTo = parseAll(sourceMODs,imputcsv)
        xml2wb_process = xml2wb(imput_csv)
main()
...
#Step2: get paths and errors:                       >>> python3 The\ Parser7.py -i Data/LDLContent -o Output/LDLContent_Paths_may19 -c Output/LDL_Content_attTags_may19.csv
#Step3: run workbench using the csv report:         >>> python3 The\ Parser7.py -i Data/LDLContent -c Output/LDLContent_Paths_may19.csv -o Output/LDLContent_Field_mapping
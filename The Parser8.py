...
def xml2workbench(DataFrame, mods):
    ...
    wb_data = {}
    for fields in DataFrame["Fields"]:
        wb_data[fields] = " "
    print(wb_data)
    field_name = []
    print(list(DataFrame["XMLPath"]))
    test = []
    for paths in result_list:
        if paths in list(DataFrame["XMLPath"]):
            ...
            wb_data[fields_text] = paths #instead of path we want text in this paths variable

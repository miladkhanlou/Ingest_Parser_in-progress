...
### 1) Getting all the tags and attributes: ###
def get_tag_attrib(mods):
    ...
    for mod in mods:
        ...
        for elem,event in root:
            if elem == 'start':
                allTags.append(event.tag.split("}")[1])
                if len(event.attrib) > 0:
...
def main():
    ...
    # Run the function to get and write the unique tags and attributes (-i,-oat)
        ...
        getAttributeTags = get_tag_attrib(sourceMODs)
        GetUniques = unique_tag_attrib()
        attribTags2csv = uniqData_toCsv(args)

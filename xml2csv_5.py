...
unique_Tag_Dict = {}         # Unique TagNames with the number of repitation in a dictionary)
unique_Attrib_Dict = {}      #(Unique Attribute with the number of repitation in a dictionary)
unique_Tag = []              #(Name of the unique attributes)
unique_Attrib = []           #(Name of the unique Unique Tags)
...
def unique_tag_attrib(tags, attributes):
    ...
    for TGs in tags:
        key = TGs
        if TGs not in tag_Check:
            tag_Check.append(TGs)
...
def uniqData_to_dict(arg):
    ...
    for att,tg in unique_Attrib_Dict.items():
        data['atributes'].append(att)
        data['atributes frequency'].append(tg)
    
    for atts,tgs in unique_Tag_Dict.items():
...
def get_csv(arg):
    ...
    df_attrib_Tags = pd.read_csv(arg.input_csv)
    column_Names = df_attrib_Tags.columns.tolist()
    data_dict =  {}
    for columns in column_Names:
        data_dict[columns] = list(df_attrib_Tags[columns])
    print(data_dict)
    return data_dict
...
class xmlSet(object):
    ...
    def xml_Mods(self, arg,dataframe):
        files = listdir(arg.input_directory)
        files.sort()
        print(files)
        for file in files:
...
def xml_parse(root,data_frame, arg):  
    ...
    if data_frame is None and not arg.input_csv:
        # print(allTags)
        # print(allAtrrib)
        return unique_tag_attrib(all_Atrrib, all_Tags)
...
def main():
    ...
    # Run the function to get and write the unique tags and attributes (-i,-oat)
        source_MODs = MODs(args,dataframe=None)
        for sourceMOD in source_MODs:
    ...
    # Run the function to get XML paths, check for errors, and write to CSV (-i,-o,-c)
        ...
        source_MODs = MODs(args, inputCSV)
        for sourceMOD in source_MODs:
            getUniquesPaths = PathRepeatCheck(sourceMOD)
            getUniqueErrors = ErrorRepeatCheck()
        write_to_CSV = paths_to_dict(getUniquesPaths, getUniqueErrors, args)
    ...
    # Run the function to for xml2wirkbencg process (-ii,-o,-cc)
        ...
        data = xmlSet()
        data.xml_Mods(args,csv_to_dict)

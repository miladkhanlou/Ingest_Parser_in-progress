    ...
    for event, elem in root:
        ...
        if event == 'start' and elem != first_elem[0]:
                ...
                if arg.input_csv or arg.input_clear_csv:
                    ...
                    if len(attributes) > 1:
                        ...
                        for i in WriteAttributes:
                            ...
                            path = '/'.join(path_name)
                            paths_with_multiple_att.append(path)
                            path_name.pop()
                        ...
                        path = '/'.join(path_name)
                        paths_with_multiple_att.append(path)
                        # print(paths_with_multiple_att)
                        # print(len(paths_with_multiple_att))
                        # print("************************************************")
            ...
            if arg.input_clear_csv and elem.text is not None and elem.text.strip() != "":
                if len(paths_with_multiple_att) > 0:
                    for n in len(paths_with_multiple_att):
                        result_dict_temp.setdefault(paths_with_multiple_att[n], [])
                        result_dict_final.setdefault(paths_with_multiple_att[n], [])
                        if elem.text.strip() not in result_dict_temp[paths]: 
                            result_dict_temp[paths].append(elem.text.strip())

                elif len(paths_with_multiple_att) == 0:
